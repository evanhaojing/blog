    ...
    def process_request(self, request):
        ...
        for check_path in not_need_check:
            if re.match(check_path, path):
                ...
                return None
            # path为需要做登录校验的路由是，判断用户是否登录，没有登录则返回登录界面
        if not user_id:
...
class SessionToDbMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request, response):
        # 同步session中的商品信息和数据中购物车表的商品信息
        # 1、判断用户是否登录，登录才做数据同步
        user_id = request.session.get('user_id')
        if user_id:
            # 2、同步
            # 2.1、判断session中的商品是否存在于数据库中，若存在，则更新
            # 2.2、若不存在，则创建
            ...
            # 组装存储的多个商品格式:[[goods_id, num, is_select],[goods_id, num, is_select],[goods_id, num, is_select]]
            if db_carts:
